...
const Historico: React.FC = () => {
  ...
  const filtered = registros.filter(r => {
      ...
      const matchFecha = r.periodo.substring(0, 7) >= filterDesde && r.periodo.substring(0, 7) <= filterHasta;
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4">
         <div className="space-y-1">
            ...
            <select value={filterProceso} onChange={e => { setFilterProceso(e.target.value); setFilterIndicador('Todos los indicadores'); }} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 outline-none">
         ...
         <div className="space-y-1">
            ...
            <input type="month" value={filterDesde} onChange={e => setFilterDesde(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 outline-none" />
         </div>
         <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Hasta</label>
            <input type="month" value={filterHasta} onChange={e => setFilterHasta(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 outline-none" />
         </div>
      </div>
            ...
            <div key={r.id} className="bg-white rounded-xl border border-slate-200 p-6 hover:shadow-md transition-all animate-in slide-in-from-bottom-2 duration-300">
                <div className="flex justify-between items-start mb-4 border-b border-slate-50 pb-4">
                    ...
                    <div className="text-right">
                        <span className="block text-3xl font-black text-slate-900 leading-none">{r.resultado_mensual}%</span>
                        <span className="text-[10px] font-bold text-slate-400 uppercase">Meta: {r.meta}%</span>
